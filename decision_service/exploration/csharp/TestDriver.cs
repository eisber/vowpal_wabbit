            ...
            using (var expected = File.OpenText(args[1]))
            {
                ...
                while ((line = input.ReadLine()) != null)
                {
                    ...
                    try
                    {
                        ...
                        {
                            case "epsilongreedy":
                                probabilityDistribution = ExplorationStrategies.EpsilonGreedy(
                                ...
                                break;
                            case "softmax":
                                probabilityDistribution = ExplorationStrategies.Softmax(
                                   float.Parse(fields[1], CultureInfo.InvariantCulture),
                                   fields.Skip(2).Select(f => float.Parse(f, CultureInfo.InvariantCulture)).ToArray());
                                break;
                            case "bag":
                                probabilityDistribution = ExplorationStrategies.Bag(
